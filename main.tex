\documentclass[12pt,a4paper]{article}
\usepackage[utf8]{inputenc}[spanish]
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{lmodern}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{enumerate}
\usepackage{graphicx}
\usepackage{mathtools}
\usepackage{stackrel}
\usepackage{multicol}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{lmodern}
\usepackage{amsmath}
\usepackage{enumerate}
\usepackage{algorithmic}
\usepackage{algorithm}
\usepackage{graphicx}
\usepackage{mathtools}
\usepackage{gensymb}
\usepackage[left=2cm,right=2cm,top=2cm,bottom=2cm]{geometry}

% Personal commands
\newcommand{\PN}{\par\noindent}
\newcommand{\VS}{\vspace{3mm}}

\providecommand{\abs}[1]{\lvert#1\rvert} % Absolute Value
\providecommand{\BV}[1]{\vec{\mathbf{#1}}} % Bold Vector


\author{Agustín Curto, agucurto95@gmail.com}
\title{Resumen para el final \\ de Física}
\date{2017}


\begin{document}
	\clearpage\maketitle
	\thispagestyle{empty}
	\tableofcontents

	\vspace{5cm}
	\PN \textbf{Nota:} Este resumen se corresponde con la materia dictada en el año 2017. Su desarollo está basado tanto
	en los libros: \cite{1} y \cite{2}, como en el apunte de clase. El autor no se responsabiliza de posibles cambios que
	pudiesen realizarse en los temas dictados en la misma, así como tampoco de errores involuntarios que pudiesen existir
	en dicho resumen.

	\bibliographystyle{splncs04}
	\bibliography{bibliography}

	\vspace{\fill}
	\begin{center}
		Por favor, mejorá este documento en github
		\includegraphics[width=1cm]{graphics/github.png} \\
		https://github.com/ResumenesFaMAF/resumenFisica
	\end{center}

	\pagebreak

	\part{Mecánica}
		\input{1/1_mecanica}
		\input{1/2_mecanica}
		\input{1/3_mecanica}
		\input{1/4_mecanica}
		\input{1/5_mecanica}
		\input{1/6_mecanica}
		\input{1/7_mecanica}
		\input{1/8_mecanica}
		\input{1/9_mecanica}
		\input{1/10_mecanica}
		\input{1/11_mecanica}
		\input{1/12_mecanica}
		\input{1/13_mecanica}

	\part{Movimiento Oscilatorio}
		\input{2/1_oscilatorio}
		\input{2/2_oscilatorio}
		\input{2/3_oscilatorio}
		\input{2/4_oscilatorio}
		\input{2/5_oscilatorio}

	\part{Termodinámica}
		\input{3/1_termodinamica}
		\input{3/2_termodinamica}
		\input{3/3_termodinamica}
		\input{3/4_termodinamica}
		\input{3/5_termodinamica}
		\input{3/6_termodinamica}
		\input{3/7_termodinamica}
		\input{3/8_termodinamica}
		\input{3/9_termodinamica}

	\part{Electricidad y Magnetismo}
		\input{4/1_electricidad}
		\input{4/2_electricidad}
		\input{4/3_electricidad}
		\input{4/4_electricidad}
		\input{4/5_electricidad}
		\input{4/6_electricidad}
\end{document}
